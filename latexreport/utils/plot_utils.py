...
def generate_subplots(figure_variables, nplots_insubfigure=4, figsize=(12, 8),
    ...
    x_key = list(figure_variables)[0]
    y_key = list(figure_variables)[1]
    plot_label_title = list(figure_variables)[2]

    filename = f'{filenametag}{plot_label_title}'
    filenames = []
    counter = 0
    fig, axs = _initiate_fig_axs(x_key, y_key, fontsize, figsize,
                                 nrows_insubplotgrid, ncols_insubplotgrid)
    if not extension.startswith('.'):
        extension = '.' + extension

    for i, (plot_label, x, y) in enumerate(
                                            zip(figure_variables[
                                                plot_label_title],
                                                figure_variables[x_key],
                                                figure_variables[y_key])):
        ...
        if plot_kw:
            if isinstance(plot_kw, list):
                axs[ax_idx].plot(x, y, label=f'{plot_label_title}{plot_label}',
                                 **plot_kw[i])
            else:
                axs[ax_idx].plot(x, y, label=f'{plot_label_title}{plot_label}',
                                 **plot_kw)
...
def generate_subbarplots(figure_variables, nplots_insubfigure=4, figsize=(12, 8),
                      fontsize=15, extension='.pdf', dpi=150,
                      nrows_insubplotgrid=1, ncols_insubplotgrid=1,
                      bar_kw=None, ax_kw=None, fig_kw=None, savedir='',
                      filenametag=''):
    """create and save x,y plots with matplotlib subplots
    ...
    dpi : int, optional
        DESCRIPTION. The default is 150.
    bar_kw : dict or list of dicts, optional
        DESCRIPTION. additional kwargs for bars
    ax_kw : dict or list of dicts, optional
        DESCRIPTION. additional kwargs for ax object
    fig_kw : dict or list of dicts, optional
        DESCRIPTION. additional kwargs for fig object
    ...
    for i, (plot_label, x, y) in enumerate(
        ...
        ax_idx = int(np.floor(counter/nplots_insubfigure))
        if bar_kw:
            if isinstance(bar_kw, list):
                axs[ax_idx].bar(x, y,
                                label=f'{plot_label_title}{plot_label}',
                                 **bar_kw[i])
            else:
                axs[ax_idx].bar(x, y,
                                label=f'{plot_label_title}{plot_label}',
